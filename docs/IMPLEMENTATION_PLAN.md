...
| --- | --- |
| 当前版本 | `0.1.0` |
| 当前阶段 | T330 Windows 原生 Codex 与 WSL Codex 双通道兼容进行中 |
...
| Windows | 原生路径、悬浮球拖动和位置保持已修复；T330 正在加入 WSL UTF-16 发行版枚举、已登录 CLI 自动选择、`wsl.exe --exec` app-server 启动、Windows/WSL 会话合并和来源诊断，等待 Windows 云端编译与真机复验 |
...
- [ ] T330（进行中）— 兼容 Windows 原生与 WSL Codex：枚举普通 WSL 发行版并解析 UTF-16 输出，自动选择已登录 CLI，通过 `wsl.exe --exec` 启动 app-server，合并 Windows/WSL 会话 Token，并显示 WSL 来源。
