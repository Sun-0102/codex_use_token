...
| --- | --- |
| 当前版本 | `0.1.0` |
| 当前阶段 | T330 Windows 原生 Codex 与 WSL Codex 双通道兼容已完成；等待 T601 双环境 Windows 真机验收 |
| 当前数据 | 账户状态/套餐来自真实 `account/read`；额度窗口来自真实 `account/rateLimits/read`；今日真实 Token 直接来自 Windows 原生与 WSL `$CODEX_HOME/sessions`（默认 `~/.codex/sessions`）和 `archived_sessions` 的 `event_msg/token_count` 数字事件，按累计快照差值还原请求并以 `input_tokens + output_tokens` 合并统计，不依赖 cc-switch；账户 Token 日桶/累计仍来自真实 `account/usage/read` 作为兜底；当前任务 Token 来自同一个应用级 app-server 长连接接收的 `thread/tokenUsage/updated`（仅当前连接可见）；全部监控项启动时立即读取并由 Tauri 后台每 30 秒发出刷新节拍，窗口隐藏后继续刷新，上一轮未结束时跳过本轮；安装版会解析 PATH、nvm、fnm、asdf、Homebrew、登录 shell、Windows 原生目录与 WSL 发行版中的 Codex CLI；账户、额度、账户 Token 和当前任务 Token 复用一个 `codex app-server --stdio` 长连接；连接失败时显示 Demo 或 stale，不标记为实时 |
| macOS | 菜单栏常驻；原生 NSPanel 浮层；点击外部自动隐藏；托盘定位拿不到 monitor 时降级到右上角显示 |
| Windows | 原生路径、悬浮球拖动和位置保持已修复；已加入 WSL UTF-16 发行版枚举、已登录 CLI 自动选择、`wsl.exe --exec` app-server 启动、Windows/WSL 会话合并和来源诊断；GitHub Actions Windows x64 测试、Clippy、release 构建和 artifact 上传已通过，等待双环境真机复验 |
...
- [x] T007：建立 Windows 收起态和 420×510 详细态切换逻辑。（收起态已由 T327 重构为 82×82 悬浮球）
...
- [x] T323 — 按参考样式重构 420×510 详细面板：使用轻量半透明卡片、单环最低余量仪表、周期/重置/套餐信息和渐变余量轨道；保留今日 Token、缓存命中、当前任务 Token，以及实时/过期缓存/演示状态区分。
- [x] T324 — 从详细面板和 Windows 收起态移除 5 小时额度，仅用周额度驱动环形仪表、进度条和周期信息；周额度缺失时显示等待同步，不以 5 小时数据冒充周额度。
...
- [x] T329 — 修复 Windows 真机数据读取和悬浮球交互：查找官方独立安装目录与 `CODEX_INSTALL_DIR`，显示完整连接诊断；以 5px 手势阈值区分点击和原生拖动，并在展开/收起、托盘隐藏/显示后保留位置。
- [x] T330 — 兼容 Windows 原生与 WSL Codex：枚举普通 WSL 发行版并解析 UTF-16 输出，自动选择已登录 CLI，通过 `wsl.exe --exec` 启动 app-server，合并 Windows/WSL 会话 Token，并显示 WSL 来源。
...
- Windows 安装版默认从 `%USERPROFILE%\.codex` 读取本地会话，并检查 `CODEX_INSTALL_DIR`、官方独立安装目录 `%LOCALAPPDATA%\Programs\OpenAI\Codex\bin`、`%APPDATA%\npm`、WindowsApps 和 Volta 中的 Codex CLI；若用户自定义目录，仍应使用 `CODEX_HOME` 或 `CODEX_CLI_PATH` 显式覆盖。
- Windows 中找不到可用原生 CLI 时会枚举 WSL 普通发行版（跳过 Docker Desktop 内部发行版），使用发行版内已登录的 Codex app-server；今日 Token 会汇总可读取的 Windows 与 WSL `$CODEX_HOME`，但同一会话目录不应同时映射成不同路径，否则可能重复统计。
...
| 2026-07-29 | T329 Windows 真机修复 | TDD 红灯复现官方独立安装目录缺失、连接诊断不含 CLI 路径、拖动阈值模块缺失和旧 CLI 文案；实现后定向 15 个前端测试与 Rust 官方目录测试通过；`npm run check` 通过（11 个前端测试文件、53 个测试、TypeScript 与 Vite 构建），Rust 全量 78 个测试、格式检查、Clippy `-D warnings`、Tauri debug 无 bundle 构建和 `git diff --check` 通过；GitHub Actions run `30414880648` 在 `windows-latest` 上通过前端检查、Windows Rust 测试、Clippy、Tauri x64 release 构建与上传，artifact `codex-reserve-windows-x64` 为 5,230,537 字节 |
| 2026-07-29 | T330 Windows/WSL 双通道 | TDD 红灯覆盖 WSL UTF-16 发行版解析与 Docker 内部发行版过滤、无 shell 插值的 WSL 启动参数、原生未登录时选择已登录 WSL、Windows/WSL 今日 Token 合并和界面来源标记；`npm run check` 通过（11 个前端测试文件、54 个测试、TypeScript 与 Vite 构建），Rust 全量 83 个测试（44 个库测试、39 个集成测试）、格式检查、Clippy `-D warnings`、Tauri debug 无 bundle 构建和 `git diff --check` 通过；GitHub Actions run `30416013085` 在 `windows-latest` 上通过前端检查、Windows Rust 测试、Clippy、Tauri x64 release 构建与上传，artifact `codex-reserve-windows-x64` 为 5,271,888 字节 |
...
| 2026-07-28 | T323 详细面板视觉重构 | `npm run check` 通过：10 个前端测试文件、46 个测试，TypeScript 与 Vite 生产构建通过；Chrome 以 420×510 视口渲染验收，页面滚动尺寸保持 420×510，主面板、额度卡、Token 卡、当前任务条与页脚均未越界；`git diff --check` 通过 |
| 2026-07-28 | T324 仅展示周额度 | `npm run check` 通过：10 个前端测试文件、49 个测试，TypeScript 与 Vite 生产构建通过；新增测试覆盖双窗口时选择周额度、缺失周额度时不回退 5 小时数据，以及仅返回一个周窗口时仍可正确识别；`git diff --check` 通过 |
...
| 2026-07-28 | T323 | 详细面板改为参考图方向的轻量磨砂样式：突出最低余量单环、双周期重置信息、套餐与渐变进度轨道，并保留今日 Token、缓存命中、当前任务 Token 及真实数据状态标记 |
| 2026-07-28 | T324 | 详细面板与 Windows 收起态不再展示 5 小时额度；主环、进度条和周期信息统一使用周额度，周额度缺失时明确等待同步 |
...
| 2026-07-29 | T606 | 新增 GitHub Actions Windows x64 自动构建流程；修复首次云端构建发现的 Unix-only 测试条件编译问题，并将 GitHub 官方 Actions 升级到 Node.js 24 运行时版本；成功后上传包含 NSIS `.exe` 与 WiX `.msi` 的 artifact |
| 2026-07-29 | T329 | 根据 Windows 真机反馈补充官方 Codex 独立安装目录，详细面板显示连接失败原因；圆球改为按住拖动、单击展开，展开/收起及托盘隐藏/显示不再强制回到右上角 |
| 2026-07-29 | T330 | Windows 原生与 WSL Codex 统一自动发现：优先可用原生 CLI，回退已登录 WSL CLI；通过 `wsl.exe` 启动 app-server，并合并两个环境的本地会话 Token |
